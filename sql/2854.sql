...
(
    SELECT
    user_id, DATE_ADD(steps_date,INTERVAL -1 DAY ) AS steps_date
    FROM Steps
) AS lg
ON lg.user_id = s.user_id AND lg.steps_date = s.steps_date
LEFT JOIN 
(
    SELECT
    user_id, DATE_ADD(steps_date,INTERVAL 1 DAY ) AS steps_date
...
ON ld.user_id = s.user_id AND ld.steps_date = s.steps_date
WHERE ld.user_id IS NOT NULL AND lg.user_id IS NOT  NULL )
